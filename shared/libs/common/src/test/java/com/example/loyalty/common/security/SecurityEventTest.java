...
class SecurityEventTest {
  ...
  void shouldRequireEventType() {
    assertThrows(
        ...
        () -> {
          SecurityEvent.builder().userId("user123").action("TEST").build();
        });
  }
  ...
  void shouldAutoSetTimestampIfNotProvided() {
    ...
    SecurityEvent event = SecurityEvent.builder().type("TEST_EVENT").build();
  ...
  void shouldAllowPartialEventData() {
    SecurityEvent event = SecurityEvent.builder().type("MINIMAL_EVENT").userId("user123").build();
