...
class DataMaskingServiceTest {
  ...
  void shouldHandleInvalidName() {
    ...
    assertEquals(
        "   ",
        dataMaskingService.maskName(
            "   ")); // Whitespace only - this doesn't get trimmed by maskName
  ...
  void shouldMaskAddress() {
    assertEquals(
        "123 *** ****, USA", dataMaskingService.maskAddress("123 Main Street, Anytown, USA"));
    assertEquals(
        "456 *** ****, Canada", dataMaskingService.maskAddress("456 Oak Ave, Toronto, Canada"));
  }
