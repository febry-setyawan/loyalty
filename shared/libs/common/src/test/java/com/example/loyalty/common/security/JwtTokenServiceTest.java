...
class JwtTokenServiceTest {
  ...
  void shouldThrowExceptionForInvalidToken() {
    ...
    // When & Then
    assertThrows(
        AuthenticationException.class, () -> jwtTokenService.validateAccessToken(invalidToken));
  }
  ...
  void shouldDetectExpiredToken() {
    ...
    JwtTokenService shortExpiryService =
        new JwtTokenService(
            "test-secret-key-for-jwt-signing-minimum-256-bits",
            "test-refresh-secret-key-for-jwt-signing-minimum-256-bits",
            1L, // 1 millisecond
    ...
    // Given
    String token =
        shortExpiryService.generateAccessToken("user123", "test@example.com", List.of("USER"));
