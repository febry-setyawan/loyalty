...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/api/v1/points' && method === 'GET') {
    const mockPoints = [
      {
    ...
    res.end(JSON.stringify({
      ...
      message: 'Mock point transactions - service is running correctly'
    }, null, 2));
    return;
  }
  ...
  if (path === '/api/v1/info' && method === 'GET') {
    ...
    res.end(JSON.stringify({
      service: 'point-service',
      version: '1.0.0',
      description: 'Point Management Service for Loyalty System',
      endpoints: [
        'GET /health - Health check',
        'GET /health/ready - Readiness probe',
        'GET /health/live - Liveness probe',
        'GET /api/v1/points - List users (mock)',
  ...
  res.end(JSON.stringify({
    ...
    path: path,
    method: method,
    availableEndpoints: ['/health', '/health/ready', '/health/live', '/api/v1/points', '/api/v1/info']
  }, null, 2));
});
...
server.listen(PORT, () => {
  ...
  console.log(`📍 Points API: http://localhost:${PORT}/api/v1/points`);
