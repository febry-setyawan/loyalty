...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/api/v1/admin' && method === 'GET') {
    const mockUsers = [
      {
  ...
  if (path === '/api/v1/info' && method === 'GET') {
    ...
    res.end(JSON.stringify({
      service: 'admin-service',
      version: '1.0.0',
      description: 'Admin Dashboard Service for Loyalty System',
      endpoints: [
        'GET /health - Health check',
        'GET /health/ready - Readiness probe',
        'GET /health/live - Liveness probe',
        'GET /api/v1/admin - List users (mock)',
  ...
  res.end(JSON.stringify({
    ...
    path: path,
    method: method,
    availableEndpoints: ['/health', '/health/ready', '/health/live', '/api/v1/admin', '/api/v1/info']
  }, null, 2));
});
