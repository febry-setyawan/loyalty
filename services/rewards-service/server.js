...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/api/v1/rewards' && method === 'GET') {
    const mockUsers = [
      {
  ...
  if (path === '/api/v1/info' && method === 'GET') {
    ...
    res.end(JSON.stringify({
      service: 'rewards-service',
      version: '1.0.0',
      description: 'Rewards Catalog Service for Loyalty System',
      endpoints: [
        'GET /health - Health check',
        'GET /health/ready - Readiness probe',
        'GET /health/live - Liveness probe',
        'GET /api/v1/rewards - List users (mock)',
  ...
  res.end(JSON.stringify({
    ...
    path: path,
    method: method,
    availableEndpoints: ['/health', '/health/ready', '/health/live', '/api/v1/rewards', '/api/v1/info']
  }, null, 2));
});
...
server.listen(PORT, () => {
  ...
  console.log(`📍 Rewards API: http://localhost:${PORT}/api/v1/rewards`);
